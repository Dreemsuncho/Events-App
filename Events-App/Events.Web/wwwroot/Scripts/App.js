...
(function (ev) {
	ev.rootPath = '';
	ev.infoMessage = '';
	ev.emailPattern = /[a-zA-Z0-9]+@[a-zA-Z0-9]+\.[a-zA-Z]+/;
	ev.emailPatternMessage = 'Login email is invalid, must match (a-z0-9)+@(a-z0-9)+.(a-z)';
...
// if anybody desire to change calendar buttons with MDBootstrap or bootstrap
/* (function (ev) {
	let refreshCalendarStyles = function () {
		$(function () {
			if (Events.IndexModel) {
				$('.dtp-buttons')["0"].childNodes.forEach(btn => {
					if (btn.localName === 'button')
						$(btn).addClass('btn btn-sm btn-primary waves-effect waves-light');
				});
			}
		});
	}
	ev.refreshCalendarStyles = refreshCalendarStyles;
}(window.Events)); */